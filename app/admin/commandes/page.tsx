  ...
  return (
    <>
      <div style={{ marginBottom: 48, borderBottom: "1px solid rgba(17,17,17,0.10)", paddingBottom: 32 }}>
        ...
        <h1 style={{ fontFamily: "var(--font-montserrat, sans-serif)", fontStyle: "normal", fontWeight: 600, fontSize: 48, lineHeight: 1 }}>Commandes</h1>
      </div>
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 28, flexWrap: "wrap" }}>
        {[undefined, "PENDING", "PAID", "SHIPPED", "DELIVERED", "CANCELLED"].map((s) => (
          <Link key={s ?? "all"} href={s ? `/admin/commandes?status=${s}` : "/admin/commandes"}
            style={{ fontFamily: "var(--font-montserrat, sans-serif)", fontSize: 11, letterSpacing: "0.2em", textTransform: "uppercase", padding: "6px 14px", border: "1px solid rgba(17,17,17,0.15)", textDecoration: "none", opacity: sp.status === s || (!sp.status && !s) ? 1 : 0.4 }}>
      ...
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        ...
        <tbody>
          {orders.map((o) => {
            ...
            return (
              <tr key={o.ref} style={{ borderBottom: "1px solid rgba(17,17,17,0.06)" }}>
                <td style={{ padding: "14px 20px", fontFamily: "var(--font-montserrat, sans-serif)", fontSize: 10, opacity: 0.6 }}>{o.ref}</td>
                <td style={{ padding: "14px 20px", fontFamily: "var(--font-montserrat, sans-serif)", fontStyle: "normal", fontSize: 17 }}>{o.customerName}</td>
                <td style={{ padding: "14px 20px", fontFamily: "var(--font-montserrat, sans-serif)", fontSize: 10 }}>{fmt(o.total)}</td>
                <td style={{ padding: "14px 20px" }}><span style={{ fontFamily: "var(--font-montserrat, sans-serif)", fontSize: 11, letterSpacing: "0.15em", textTransform: "uppercase", color: s.color }}>{s.label}</span></td>
                <td style={{ padding: "14px 20px", fontFamily: "var(--font-montserrat, sans-serif)", fontSize: 11, opacity: 0.4 }}>{o.createdAt.toLocaleDateString("fr-FR")}</td>
                <td style={{ padding: "14px 20px" }}>
                  <Link href={`/admin/commandes/${o.ref}`} style={{ fontFamily: "var(--font-montserrat, sans-serif)", fontSize: 11, letterSpacing: "0.18em", textTransform: "uppercase", opacity: 0.5, textDecoration: "none" }}>Voir →</Link>
                </td>
              </tr>
            );
          })}
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 28 }}>
          {Array.from({ length: pages }, (_, i) => i + 1).map((p) => (
            <Link key={p} href={`/admin/commandes?page=${p}${sp.status ? `&status=${sp.status}` : ""}`}
              style={{ fontFamily: "var(--font-montserrat, sans-serif)", fontSize: 10, padding: "6px 12px", border: "1px solid rgba(17,17,17,0.15)", textDecoration: "none", opacity: p === page ? 1 : 0.4 }}>{p}</Link>
          ))}
        </div>
      )}
    </>
